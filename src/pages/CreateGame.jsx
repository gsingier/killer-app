...
import { Volume2, Check, Plus, Trash2, ArrowLeft } from 'lucide-react';
...
export default function CreateGame() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedCategories.length === 0 && customMissions.filter(m => m.trim()).length === 0) {
      showNotification('Veuillez sélectionner au moins une catégorie ou ajouter un mot secret.', 'error');
      return;
    }
    ...
    <div className="max-w-xl mx-auto p-4 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Configurez les mots secrets à faire dire</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Mission Word Categories */}
        <div className="p-5 rounded-3xl glass-card space-y-4">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
              <Volume2 className="w-4 h-4 text-amber-400" /> Catégories de Mots Secrets
            </label>
        ...
        {/* Custom Mission Words */}
        <div className="p-5 rounded-3xl glass-card space-y-4">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
              Mots Sur-Mesure Personnalisés (Optionnel)
            </label>
            <button
              type="button"
            ...
            {customMissions.map((mission, index) => (
              <div key={index} className="flex items-center gap-2">
                <input
                  ...
                  placeholder={`Mot sur-mesure #${index + 1} (ex: Tournesol)...`}
